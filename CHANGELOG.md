...
- **双通道风格注入**：风格表达走 user 自然语言通道（官方唱歌风格通道，默认 `prompt` 模式）——LLM 产出**画面感演唱描述**（"像…一样"+节奏收尾，官方示例同构）注入控制指令；`core/style_lib.py` 内置官方风格词表（6 类）与标签漏斗（`style_tags` 显式 > 本地词表提取 > LLM 筛选白名单），`sing_style_source` 三态（`prompt` 默认 / `tag` 实验 / `off`）；
...
- 多风格括号 `(唱歌 词…)` 在唱歌模式朗读：官方唱歌模式仅支持 `(唱歌)` 一种标签，风格由 user 自然语言通道表达——默认 `prompt` 模式，`tag` 保留为实验（如遇朗读请切 `prompt`）；
- 润色链路配置读取异常：补全缺失的配置属性访问，超时 / 缓存配置实际生效。

### 行为说明

- 唱歌默认走 user 自然语言风格通道（官方唱歌风格通道）；`tag` 标签注入为实验特性；
