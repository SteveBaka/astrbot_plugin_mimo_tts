...
唱歌模式增强正式版（v2.2.0 十五轮迭代合并固化的版本号推进；设计文档 `docs/sing-mode-feature.md` V1 全量落地 + 多轮实测打磨）。
...
- **导演模式先导：风格示例池**（§14，design 优先）：新配置 `style_examples`（JSON，内置 6 类高质量中文画面感示例）——design 音色描述命中风格词时自动并入"参考示例：…"（如"像融化的棉花糖一样温柔"），零 LLM 零延迟；无匹配零注入；示例池未来由导演模式按场景动态调配并覆盖 clone/唱歌/普通 TTS；
- **`design_voice_description` 支持精确引用示例池分类名**（§14.5 方案 A）：填 name（如"温柔甜美"/"磁性低沉"）→ 条目直取，用该条目完整官方词生成词表提示（修复简写缺词："磁性低沉"→"整体语调深沉，音色定位磁性"）+ 例句注入，快速切换风格；填自由描述仍走自动匹配链路（行为不变）；
...
- 多风格括号 `(唱歌 词…)` 在唱歌模式朗读：**实测矩阵（v2.2.0，五种括号写法 × 直连服务端）**——`(唱歌 温柔 甜美)`/`(唱歌 温柔，甜美)` 朗读+异常发音、`(唱歌)(温柔)(甜美)` 前半段杂音唱歌、`(唱歌 温柔)` 感情朗读、仅 `(唱歌)` 正常唱歌。唱歌模式只认精确 `(唱歌)` 标签，风格一律走 user 自然语言通道——`tag` 模式收窄为仅收集不注入（防朗读），`style_tags`/官方词表漏斗保留供未来非唱歌场景复用；
