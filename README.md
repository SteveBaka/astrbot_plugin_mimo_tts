...
- [x]8 个预设：一键切换风格
- [x]方言支持：四川话、粤语、东北话等
- [x]唱歌模式：支持 `/sing [音色名] <歌词>` 临时触发，可配置默认唱歌音色
- [x]笑声 / 停顿 / 呼吸声 / 重音模式
- [x]音量控制：轻声/正常/大声
...
> 唱歌模式仅由 `/sing` 单次触发。支持可选音色名前缀指定内置音色，未指定时使用插件设置面板中的「🎤 唱歌默认音色」配置。唱歌时自动锁定为 `mimo-v2.5-tts` 模型和内置音色，不受当前 clone/design 音色设置影响。
...
- 2026年4月28日更新：
  ...
  - 消除情感检测逻辑重复，优化代码结构；
  - 今日更新使用MiMo-V2.5对代码和功能进行优化。
  - 2026年4月29日，v1.3.0更新：
  - 新增 `sing_voice` 配置项，可在插件设置面板中选择唱歌默认音色；
  - `/sing` 命令现支持 `/sing [音色名] <歌词>` 格式，可临时指定内置音色；
  - 唱歌模式音色锁定逻辑重构，确保始终使用内置音色和 `mimo-v2.5-tts` 模型；
  - 修复 `/sing` 唱歌模式在自定义音色下音色偏移的问题；
  - 修复路径穿越风险、数值类型防御、注册表损坏处理等问题；
  - 临时音频文件存储迁移到 AstrBot 数据目录；
  - 该版本使用mimo-v2.5-pro进行更新。
