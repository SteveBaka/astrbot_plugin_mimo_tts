...
> 唱歌模式仅由 `/sing` 单次触发，执行后自动恢复原始设置，避免普通即时合成与自动语音输出被持续污染。支持通过 `-音色名` 临时指定唱歌音色（如 `/sing -冰糖 小星星`），优先级：命令参数 > 插件配置 `sing_voice` > 当前音色。
...
- 2026年4月28日更新：
  ...
  - 消除情感检测逻辑重复，优化代码结构；
  - 今日更新使用 mimo-v2.5-pro 对代码和功能进行优化。
- 2026年4月30日更新：
  ...
  - 唱歌音色优先级：命令参数 > 插件配置 `sing_voice` > 当前用户音色；
  - 使用 mimo-v2.5-pro 生成。
